...
public class InvokAll {
    ...
    public void getRankedTravelQuotes() throws InterruptedException {
        List<InvokAll.QuoteTask> tasks = new ArrayList<InvokAll.QuoteTask>();
        // 模拟10个计算旅游报价的任务
        for (int i = 1; i <= 20; i++) {
            tasks.add(new InvokAll.QuoteTask(200, i));
        }
        ...
        Iterator<InvokAll.QuoteTask> taskIter = tasks.iterator();

        for (Future<BigDecimal> future : futures) {
            InvokAll.QuoteTask task = taskIter.next();
            try {
    ...
    public static void main(String[] args) {
        try {
            CallAndFulture it = new CallAndFulture();
